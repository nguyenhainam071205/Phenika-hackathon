...
import AIAnnotationStore, { AIDetection, MOCK_AI_RESULTS, RAG1APIResponse } from './AIAnnotationStore';
import {
...
function commandsModule({
  ...
  const AI_OVERLAY_ID = 'ohif-ai-overlay-layer';

  // Module-level state for event listener management
  let _aiViewportId: string | null = null;
  let _aiEventElement: HTMLElement | null = null;

  const _getAIViewport = () => {
    if (!_aiViewportId) return null;
    return cornerstoneViewportService.getCornerstoneViewport(_aiViewportId) ?? null;
  };

  /** Remove the overlay div from the viewport element */
  const _clearAIOverlay = () => {
    const vp = _getAIViewport();
    if (!vp) return;
    (vp.element as HTMLElement).querySelector(`#${AI_OVERLAY_ID}`)?.remove();
  };

  /**
   * Render AI bounding boxes directly on the viewport element.
   * Uses worldToCanvas so boxes track zoom/pan precisely.
   * Falls back to normalised % coords when worldPoints are unavailable.
   */
  const _renderAIOverlay = (detections: AIDetection[], viewport?: any) => {
    const vp = viewport ?? _getAIViewport();
    if (!vp) return;

    const host = vp.element as HTMLElement;
    // Ensure the host can contain absolutely positioned children
    if (getComputedStyle(host).position === 'static') {
      host.style.position = 'relative';
    }

    // Remove previous overlay
    host.querySelector(`#${AI_OVERLAY_ID}`)?.remove();

    const layer = document.createElement('div');
    layer.id = AI_OVERLAY_ID;
    ...
    detections.forEach(det => {
      if (!det.visible) return;

      let left: number, top: number, width: number, height: number;

      if (det.worldPoints && det.worldPoints.length >= 3) {
        // worldPoints[0] = topLeft, worldPoints[2] = bottomRight
        try {
          const [c1x, c1y] = vp.worldToCanvas(det.worldPoints[0] as CoreTypes.Point3);
          const [c3x, c3y] = vp.worldToCanvas(det.worldPoints[2] as CoreTypes.Point3);
          left = Math.min(c1x, c3x);
          top = Math.min(c1y, c3y);
          width = Math.abs(c3x - c1x);
          height = Math.abs(c3y - c1y);
        } catch {
          return; // worldToCanvas failed — skip this box
        }
      } else {
        // Fallback: normalised coords → pixel coords
      ...
      const colors = det.confidence < 0.25 
        ? { border: '#ef4444', bg: 'rgba(239, 68, 68, 0.1)', label: 'rgba(239, 68, 68, 0.95)' }
        : det.confidence < 0.5 
          ? { border: '#f59e0b', bg: 'rgba(245, 158, 11, 0.1)', label: 'rgba(245, 158, 11, 0.95)' }
          : { border: '#22d3ee', bg: 'rgba(34, 211, 238, 0.08)', label: 'rgba(8, 145, 178, 0.95)' };

      const labelEl = document.createElement('div');
      labelEl.textContent = `[AI] ${det.label} ${Math.round(det.confidence * 100)}%`;
      labelEl.style.position = 'absolute';
      labelEl.style.left = `${left}px`;
      labelEl.style.top = `${top - 20}px`;
      ...
      labelEl.style.pointerEvents = 'none'; // label shouldn't block drags
      labelEl.style.transition = 'left 0.05s, top 0.05s';
      ...
      box.style.boxShadow = '0 0 0 1px rgba(0,0,0,0.7) inset';
      box.style.background = colors.bg;
      box.style.borderRadius = '3px';
      box.style.transition = 'left 0.05s, top 0.05s, width 0.05s, height 0.05s';

      // --- NEW DRAG & RESIZE INTERACTIVITY ---
      box.style.pointerEvents = 'auto'; // allow user interaction!
      box.style.cursor = 'move';
      box.style.resize = 'both'; // enable CSS native resizing
      box.style.overflow = 'hidden';

      let isDragging = false;
      let startX = 0, startY = 0;
      let initialLeft = left, initialTop = top;
      let currentLeft = left, currentTop = top;

      const saveCoordinates = (newLeft: number, newTop: number, newWidth: number, newHeight: number) => {
        const { clientWidth: w, clientHeight: h } = host;
        let newWorldPoints;
        try {
          const wp1 = vp.canvasToWorld([newLeft, newTop]);
          const wp2 = vp.canvasToWorld([newLeft + newWidth, newTop + newHeight]);
          newWorldPoints = [
            wp1,
            [wp2[0], wp1[1], wp1[2]],
            wp2,
        ...
        AIAnnotationStore.updateDetectionBounds(det.id, {
          x: newLeft / w,
          y: newTop / h,
          width: newWidth / w,
          height: newHeight / h,
      ...
      box.addEventListener('mousedown', (e) => {
        e.stopPropagation(); // prevent cornerstone from panning
        const rect = box.getBoundingClientRect();
        // Resize handle is typically at the bottom-right corner (~15px)
        ...
        if (isResize) {
          const onMouseUp = () => {
            ...
            saveCoordinates(currentLeft, currentTop, newW, newH);
        ...
        box.style.transition = 'none'; // smoother dragging without CSS transition
        ...
        const onMouseMove = (ev: MouseEvent) => {
          ...
          labelEl.style.left = `${currentLeft}px`;
          labelEl.style.top = `${currentTop - 20}px`;
        };
  ...
  /** Re-render overlay using the current store + registered viewport */
  const _renderAIOverlayFromState = () => {
    const vp = _getAIViewport();
    if (!vp) return;
    _renderAIOverlay(AIAnnotationStore.getDetections(), vp);
  };

  /** Attach Cornerstone events so overlay updates on every camera/image change */
  const _setupAIEvents = (viewportId: string, element: HTMLElement) => {
    _cleanupAIEvents();
    _aiViewportId = viewportId;
    _aiEventElement = element;
    element.addEventListener(CoreEnums.Events.IMAGE_RENDERED as string, _renderAIOverlayFromState);
    element.addEventListener(CoreEnums.Events.CAMERA_MODIFIED as string, _renderAIOverlayFromState);
  };

  /** Remove event listeners and clear viewport tracking */
  const _cleanupAIEvents = () => {
    if (_aiEventElement) {
      _aiEventElement.removeEventListener(
        CoreEnums.Events.IMAGE_RENDERED as string,
        _renderAIOverlayFromState
      );
      _aiEventElement.removeEventListener(
        CoreEnums.Events.CAMERA_MODIFIED as string,
        _renderAIOverlayFromState
      );
    }
    _aiViewportId = null;
    _aiEventElement = null;
  };
  ...
  const actions = {
    ...
    /**
     * Run real AI analysis by calling RAG1 backend which fetches DICOM from
     * Orthanc, runs YOLO detection + RAG retrieval + LLM generation, and
     * renders resulting bounding boxes on the active viewport.
     */
    runAIAnalysis: async () => {
      const RAG1_API_URL = '/rag1';
      ...
      if (!imageId) {
        uiNotificationService.show({
          title: 'AI Analysis',
          message: 'Cannot get image information from the current viewport.',
          type: 'error',
        });
        return;
      }

      // Extract IDs using Cornerstone metadata directly for 100% reliability
      let sopInstanceUID: string | undefined;
      let studyInstanceUID = 'unknown';
      let seriesInstanceUID: string | undefined;

      const instance = metaData.get('instance', imageId);
      if (instance) {
        sopInstanceUID = instance.SOPInstanceUID;
        studyInstanceUID = instance.StudyInstanceUID || 'unknown';
      ...
      if (!sopInstanceUID) {
        uiNotificationService.show({
          title: 'AI Analysis',
          message: 'Cannot determine SOPInstanceUID for the current image. Make sure the image is loaded from Orthanc.',
          type: 'error',
        });
        return;
      }
      ...
      uiNotificationService.show({
        title: 'AI Analysis',
        message: 'Sending image to AI pipeline (YOLO + RAG + LLM)... This may take 15-30 seconds.',
        type: 'info',
        duration: 30000,
      });

      // Clear existing AI annotations if any
      const existingDetections = AIAnnotationStore.getDetections();
      existingDetections.forEach(det => {
        if (det.annotationUID) {
          try {
            annotation.state.removeAnnotation(det.annotationUID);
          } catch {
            // ignore if already removed
          }
        }
      });
      AIAnnotationStore.clear();

      // ── Call RAG1 backend API ──
      let apiResponse: RAG1APIResponse;
      try {
        // RAG1_API_URL is already '/rag1', so we just call '/run-orthanc'
        const url = `${RAG1_API_URL}/run-orthanc`;
        console.log(`[AI Analysis] Fetching from: ${url}`);
        ...
        uiNotificationService.show({
          ...
          message: `Could not connect to RAG1 backend: ${err.message || err}. Make sure the backend is running at ${RAG1_API_URL}`,
          type: 'error',
          duration: 8000,
        });
        console.error('[AI Analysis] Backend call failed:', err);
        return;
      }

      // Store the full RAG result for the report modal
      AIAnnotationStore.setRAGResult(apiResponse);

      // ── Build AIDetections from the RAG1 response ──
      const ragResult = apiResponse.result;
      const element = viewport.element as HTMLElement;
      const { clientWidth: canvasWidth, clientHeight: canvasHeight } = element;

      // Get image dimensions from the DICOM metadata if available
      // The YOLO detections use pixel coordinates in the input JSON
      // We need to map bbox_norm (normalized 0-1) from the RAG1 request to canvas coords
      // Group findings by laterality to avoid overlapping boxes
      const groupedDetections: Record<string, AIDetection> = {};

      ragResult.results_per_detection.forEach((det, index) => {
        const laterality = det.laterality?.toLowerCase() || 'central';

        // Use real bbox_norm from the API: [x1, y1, x2, y2]
        // If not available (old API), fallback to estimates
        let x = 0.3, y = 0.3, width = 0.2, height = 0.2;
        
        const feFinding = ragResult.final_for_fe?.findings?.find(f => f.det_id === det.det_id);
        const bbox_norm = feFinding?.bbox_norm || (det as any).bbox_norm;

        if (bbox_norm && bbox_norm.length === 4) {
          x = bbox_norm[0];
          y = bbox_norm[1];
          width = bbox_norm[2] - bbox_norm[0];
          height = bbox_norm[3] - bbox_norm[1];
        } else {
          // Fallback to regional estimates if bbox_norm is missing
          if (laterality === 'right') {
            x = 0.15; y = 0.25; width = 0.25; height = 0.3;
          } else if (laterality === 'left') {
            x = 0.6; y = 0.25; width = 0.25; height = 0.3;
          } else if (laterality === 'central') {
            x = 0.3; y = 0.2; width = 0.4; height = 0.4;
          }
        }

        const confidence = feFinding?.confidence ?? det.findings_draft?.severity_confidence ?? 0.5;
        const key = `${x}-${y}-${width}-${height}`;

        if (groupedDetections[key]) {
          // If already exists, append label and update max confidence
          if (!groupedDetections[key].label.includes(det.class_name)) {
            groupedDetections[key].label += `, ${det.class_name}`;
          }
          groupedDetections[key].confidence = Math.max(groupedDetections[key].confidence, confidence);
          if (!groupedDetections[key].ragDetIds) groupedDetections[key].ragDetIds = [];
          groupedDetections[key].ragDetIds.push(det.det_id);
        } else {
          // Create new grouped detection
          groupedDetections[key] = {
            id: `ai-group-${laterality}-${index}`,
            label: det.class_name,
            confidence,
            x, y, width, height,
            visible: true,
            annotationUID: undefined,
            ragDetIds: [det.det_id],
          };
        }
      });

      const detections = Object.values(groupedDetections);

      // Use Cornerstone 3D utilities for pixel-perfect image-to-world coordinate mapping
      const imageData = viewport.getImageData();
      let imgWidth = 1000, imgHeight = 1000;

      if (imageData && imageData.dimensions) {
        imgWidth = imageData.dimensions[0];
        imgHeight = imageData.dimensions[1];
      }

      const getFractionalWorldCoord = (u: number, v: number): CoreTypes.Point3 => {
        // Map fractional coordinate to raw DICOM image pixel coordinates
        const pixelX = u * imgWidth;
        const pixelY = v * imgHeight;
        // Cornerstone3D natively transforms index [col, row] into 3D world space
        return csUtils.imageToWorldCoords(imageId, [pixelX, pixelY]);
      };

      // Create RectangleROI annotations in Cornerstone
      detections.forEach(det => {
        const worldPt1 = getFractionalWorldCoord(det.x, det.y);
        const worldPt2 = getFractionalWorldCoord(det.x + det.width, det.y);
        const worldPt3 = getFractionalWorldCoord(det.x + det.width, det.y + det.height);
        const worldPt4 = getFractionalWorldCoord(det.x, det.y + det.height);
        ...
        const newAnnotation: ToolTypes.Annotation = {
          ...
          data: {
            label: `[AI] ${det.label} (${Math.round(det.confidence * 100)}%)`,
            handles: {
              points: [worldPt1, worldPt2, worldPt3, worldPt4],
        ...
        const annotationColor = det.confidence < 0.25 
          ? 'rgb(239, 68, 68)' 
          : det.confidence < 0.5 
            ? 'rgb(245, 158, 11)' 
            : 'rgb(0, 255, 255)';
        ...
        det.annotationUID = newAnnotation.annotationUID;
      });

      // Persist to store so the panel can render it
      AIAnnotationStore.setDetections(studyInstanceUID, detections);

      // Register viewport so overlay auto-updates on zoom/pan/scroll
      _setupAIEvents(viewportId, element);

      // Initial render using worldToCanvas
      _renderAIOverlay(detections, viewport);

      // Trigger re-render
      const renderingEngine = cornerstoneViewportService.getRenderingEngine();
      renderingEngine.renderViewport(viewportId);

      const processingTime = ragResult.metadata?.processing_time_ms
        ? `(${(ragResult.metadata.processing_time_ms / 1000).toFixed(1)}s)`
        : '';

      uiNotificationService.show({
        title: 'AI Analysis Complete',
        message: `Found ${detections.length} abnormalities. Overall severity: ${ragResult.overall_impression?.overall_severity || 'N/A'} ${processingTime}`,
        type: ragResult.overall_impression?.requires_urgent_action ? 'warning' : 'success',
        duration: 5000,
      });
    },

    /**
     * Toggle visibility of a single AI detection annotation.
     */
    toggleAIAnnotationVisibility: ({ detectionId }: { detectionId: string }) => {
      const det = AIAnnotationStore.getDetections().find(d => d.id === detectionId);
      if (!det?.annotationUID) {
        return;
      }
      AIAnnotationStore.toggleDetectionVisibility(detectionId);
      const ann = annotation.state.getAnnotation(det.annotationUID);
      if (ann) {
        // Cornerstone uses visibility API
        const newVisible = AIAnnotationStore.getDetections().find(
          d => d.id === detectionId
        )?.visible;
        annotation.visibility.setAnnotationVisibility(det.annotationUID, newVisible ?? true);
        _renderAIOverlayFromState();
        const renderingEngine = cornerstoneViewportService.getRenderingEngine();
        renderingEngine.render();
      }
    ...
    /**
     * Remove a single AI detection annotation.
     */
    removeAIAnnotation: ({ detectionId }: { detectionId: string }) => {
      const det = AIAnnotationStore.getDetections().find(d => d.id === detectionId);
      if (det?.annotationUID) {
        try {
          annotation.state.removeAnnotation(det.annotationUID);
      ...
      AIAnnotationStore.removeDetection(detectionId);
      _renderAIOverlayFromState();
      const renderingEngine = cornerstoneViewportService.getRenderingEngine();
      renderingEngine.render();
    },
    ...
    /**
     * Update the label of an AI detection.
     */
    updateAIAnnotationLabel: ({ detectionId, newLabel }: { detectionId: string; newLabel: string }) => {
      const det = AIAnnotationStore.getDetections().find(d => d.id === detectionId);
      if (!det) {
        return;
      }
      AIAnnotationStore.updateDetectionLabel(detectionId, newLabel);

      // Also update the Cornerstone annotation label
      if (det.annotationUID) {
        const ann = annotation.state.getAnnotation(det.annotationUID);
        if (ann) {
          ann.data.label = `[AI] ${newLabel} (${Math.round(det.confidence * 100)}%)`;
    ...
    /**
     * Clear ALL AI annotations from the viewport and store.
     */
    clearAllAIAnnotations: () => {
      AIAnnotationStore.getDetections().forEach(det => {
        if (det.annotationUID) {
          try {
            annotation.state.removeAnnotation(det.annotationUID);
          } catch {
    ...
    /**
     * Confirm and save AI results (show notification / stub for real save).
     */
    confirmAIResults: () => {
      const detections = AIAnnotationStore.getDetections();
      if (detections.length === 0) {
        uiNotificationService.show({
          title: 'Xác nhận kết quả',
          message: 'Không có kết quả AI nào để xác nhận.',
          type: 'warning',
        });
        return;
      }
      uiNotificationService.show({
        title: 'Xác nhận kết quả',
        message: `Đã lưu ${detections.length} kết quả AI vào báo cáo.`,
        type: 'success',
        duration: 4000,
      });
      console.log(
        '[AI] Confirmed results:',
  ...
  const definitions = {
    ...
    // AI Analysis commands
    runAIAnalysis: { commandFn: actions.runAIAnalysis },
    toggleAIAnnotationVisibility: { commandFn: actions.toggleAIAnnotationVisibility },
    removeAIAnnotation: { commandFn: actions.removeAIAnnotation },
    updateAIAnnotationLabel: { commandFn: actions.updateAIAnnotationLabel },
    clearAllAIAnnotations: { commandFn: actions.clearAllAIAnnotations },
    confirmAIResults: { commandFn: actions.confirmAIResults },
  };
