...
function getStudyTimestamp(study) {
  const rawDate = (study?.date || '').replace(/[^0-9]/g, '');
  const rawTime = (study?.time || '').replace(/[^0-9]/g, '');

  if (rawDate.length < 8) {
    return 0;
  }

  const normalizedDate = rawDate.slice(0, 8);
  const normalizedTime = (rawTime + '000000').slice(0, 6);

  return Number(`${normalizedDate}${normalizedTime}`);
}
...
function processResults(qidoStudies) {
  ...
  // Keep newest studies (typically newly uploaded) at the top of the worklist.
  studies.sort((a, b) => getStudyTimestamp(b) - getStudyTimestamp(a));
...
export function seriesInStudy(dicomWebClient, studyInstanceUID) {
  return dicomWebClient.searchForSeries({ studyInstanceUID });
}
