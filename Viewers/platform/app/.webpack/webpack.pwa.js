...
module.exports = (env, argv) => {
  ...
  if (hasProxy) {
    // Derive the Orthanc root path prefix (e.g. '/pacs' from '/pacs/dicom-web')
    const orthancPathPrefix = PROXY_PATH_REWRITE_FROM && PROXY_PATH_REWRITE_TO
      ? PROXY_PATH_REWRITE_FROM.slice(0, PROXY_PATH_REWRITE_FROM.lastIndexOf(PROXY_PATH_REWRITE_TO))
      : null;

    const existingProxy = mergedConfig.devServer.proxy || [];
    
    // Standardizing existingProxy to array if it is not
    const proxyArray = Array.isArray(existingProxy) ? existingProxy : [existingProxy];
    ...
    mergedConfig.devServer.proxy = [
      ...
      // Broader Orthanc REST API path
      ...(orthancPathPrefix
        ? [
            {
              context: [orthancPathPrefix],
              target: PROXY_DOMAIN,
              changeOrigin: true,
              pathRewrite: {
                [`^${orthancPathPrefix}`]: '',
